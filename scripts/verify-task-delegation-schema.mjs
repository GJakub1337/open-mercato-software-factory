...
import { Migration20260919121846_task_delegation } from '../src/modules/task_delegation/migrations/Migration20260919121846_task_delegation.ts'

const databaseUrl = process.env.TASK_DELEGATION_TEST_DATABASE_URL
assert.ok(databaseUrl, 'Set TASK_DELEGATION_TEST_DATABASE_URL to the disposable PostgreSQL test database')
...
try {
  ...
  const statements = []
  await Migration20260919121846_task_delegation.prototype.up.call({ addSql: (sql) => statements.push(sql) })
  ...
  const insertDelegation = `INSERT INTO task_delegations
  ...
  await assert.rejects(client.query(insertDelegation, values), { code: '23505', constraint: 'task_delegations_active_task_uq' })
  ...
  await client.query('UPDATE task_delegations SET released_at = now() WHERE id = $1', [first.rows[0].id])
  await client.query(insertDelegation, values)
  const processId = randomUUID()
  const receipt = `INSERT INTO task_delegation_process_writes
    (tenant_id, organization_id, task_id, process_instance_id, step_id, command_id, created_at, updated_at)
    VALUES ($1, $2, $3, $4, 'step-one', 'task_delegation.task.link', now(), now())`
  ...
  await assert.rejects(client.query(receipt, receiptValues), { code: '23505', constraint: 'task_delegation_process_writes_step_uq' })
