...
**Context**: Task delegation passed 65 mocked unit tests, then failed on every request in the built app. The generated commands and interceptors bundles each inlined `tasks/lib/columnContext.ts`, and the Next server chunks each inlined `@open-mercato/shared` `commands/transaction.js` (from a since-dropped patch). Each copy kept its own module-level `WeakMap`, so state written by one command was invisible to the guard or service reading it. The same run also exposed CLASSIC DI injection ignoring a destructured `{ em }` factory, and subscribers reading a `hasRegistration` the event bus never passes.
...
**Applies to**: `src/modules/*/lib` state shared between commands, interceptors, and subscribers; `di.ts` registrations; subscriber tests.
