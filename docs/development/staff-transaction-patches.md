...
The shared patch also keeps the managed-transaction registry on `globalThis` (`Symbol.for('open-mercato.shared.managed-command-transactions')`), an app-side amendment to `d04c65202`. The Next production build inlines `@open-mercato/shared` into more than one server chunk. With a module-level `WeakMap`, staff's `lockTask` in one chunk could not see a transaction the command bus opened in another, and every delegation over HTTP failed with `Staff task mutation requires a managed command transaction`. Carry this into the upstream Core change.

The verifier compares installed source and runtime hashes with `staff-transaction-patches.json`, checks that Core and the app resolve the same shared command registry, exercises the compiled commit/effect ordering, and checks that a second module instance sees the same transaction registry. The Core source change separately passed 19 isolated PostgreSQL regressions, 181 shared command tests and 88 selected Core tests, package builds, typechecks, and correctness/security re-review. These checks do not certify application delegation, full CI, paid agent execution or deployment.
