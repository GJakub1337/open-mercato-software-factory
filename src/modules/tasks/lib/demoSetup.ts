...
export type TasksDemoScope = { tenantId: string; organizationId: string }

export type TasksDemoResult = {
  customerId: string
...
type AgentPrincipalService = {
  provision(scope: TasksDemoScope, input: { agentDefinitionId: string; displayName?: string; roleFeatures?: string[] }): Promise<{ userId: string }>
}

function systemContext(container: AwilixContainer, scope: TasksDemoScope): CommandRuntimeContext {
  return {
    container,
...
async function firstId(qe: QueryEngine, entity: string, filters: Record<string, unknown>, scope: TasksDemoScope): Promise<string | null> {
...
async function resolveAdminUser(em: EntityManager, scope: TasksDemoScope, email?: string): Promise<User | null> {
...
export async function seedTasksDemo(
  ...
  scope: TasksDemoScope,
  options: { adminEmail?: string } = {},
): Promise<TasksDemoResult> {
  ...
  if (typeof hasRegistration === 'function' && hasRegistration.call(container, 'agentPrincipalService')) {
    const principal = await container.resolve<AgentPrincipalService>('agentPrincipalService').provision(scope, {
      ...
      displayName: 'Factory',
      roleFeatures: ['tasks.view', 'tasks.process'],
    })
