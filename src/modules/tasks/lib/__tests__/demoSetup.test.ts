...
import { seedTasksDemo } from '../demoSetup'
...
it('creates the Internal customer, admin staff member, DEMO project, factory columns and agent', async () => {
  const result = await seedTasksDemo(container(true), scope)
  ...
  expect(provision).toHaveBeenCalledWith(scope, expect.objectContaining({ agentDefinitionId: 'factory', roleFeatures: ['tasks.view', 'tasks.process'] }))
...
it('changes nothing when every record already exists', async () => {
  ...
  const result = await seedTasksDemo(container(false), scope)
