...
const USAGE = 'Usage: mercato tasks seed-demo --tenant <tenantId> --org <organizationId> [--admin <email>]'
...
const seedDemo: ModuleCli = {
  ...
  async run(rest) {
    ...
    const result = await seedTasksDemo(container, { tenantId, organizationId }, { adminEmail: readFlag(rest, 'admin') })
    console.log(
      `Tasks demo (org=${organizationId}): project DEMO ${result.projectId}, ` +
