...
export async function withTaskRoute(
  ...
  const denied = (status: number) => Response.json({ error: translate('task_delegation.errors.forbidden') }, { status })
  ...
  try {
    ...
    if (error instanceof z.ZodError) return Response.json({ error: translate('task_delegation.errors.validation'), fieldErrors: error.flatten().fieldErrors }, { status: 400 })
    throw error
  }
}
