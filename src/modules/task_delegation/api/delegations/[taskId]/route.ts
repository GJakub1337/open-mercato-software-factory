...
export const metadata = { DELETE: { requireAuth: true, requireFeatures: ['task_delegation.delegate'] } }
export async function DELETE(request: Request, route: { params: Promise<{ taskId: string }> }) {
  return withTaskRoute(request, ['task_delegation.delegate'], async ({ commandContext, userId, tenantId, organizationId, userFeatures }) => {
    ...
    const executed = await bus.execute<UndelegateTaskInput, UndelegateTaskResult>('task_delegation.task.undelegate', { input: guarded, ctx: commandContext })
