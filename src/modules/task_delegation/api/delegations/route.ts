...
export const metadata = {
  GET: { requireAuth: true, requireFeatures: ['task_delegation.view'] },
  POST: { requireAuth: true, requireFeatures: ['task_delegation.delegate'] },
}
...
export async function POST(request: Request) {
  return withTaskRoute(request, ['task_delegation.delegate'], async ({ commandContext, userId, tenantId, organizationId, userFeatures }) => {
    ...
    const executed = await bus.execute<DelegateTaskInput, DelegateTaskResult>('task_delegation.task.delegate', { input: guarded, ctx: commandContext })
