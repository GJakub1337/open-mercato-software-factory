...
const request = new Request('http://localhost/api/task_delegation/delegations')
...
beforeEach(() => {
  ...
  mockGranted.mockResolvedValue(['task_delegation.*'])
})
...
it.each([
  ...
  const result = await withTaskRoute(request, ['task_delegation.view'], handler)
...
it('fails closed when the required feature is denied', async () => {
  ...
  const handler = jest.fn<(ctx: TaskRouteContext) => Promise<Response>>()
  expect((await withTaskRoute(request, ['task_delegation.delegate'], handler)).status).toBe(403)
  expect(handler).not.toHaveBeenCalled()
})
...
it('passes trusted selected scope and wildcard features to the handler', async () => {
  const handler = jest.fn(async (ctx: TaskRouteContext) => {
    ...
    expect(ctx.userFeatures).toEqual(['task_delegation.*'])
    return Response.json({ ok: true })
  })
  expect((await withTaskRoute(request, ['task_delegation.view'], handler)).status).toBe(200)
  expect(mockAllowed).toHaveBeenCalledWith('user', ['task_delegation.view'], { tenantId: 'tenant', organizationId: 'organization' })
})
