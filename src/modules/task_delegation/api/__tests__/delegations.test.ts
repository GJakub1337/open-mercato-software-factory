...
const mockContext = {
  ...
  userId: 'actor', tenantId: 'tenant', organizationId: 'organization', userFeatures: ['task_delegation.*'],
}
...
function post(body: unknown) {
  return POST(new Request('http://localhost/api/task_delegation/delegations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }))
}

it('performs one scoped delegation read for a deduplicated batch', async () => {
  expect((await GET(new Request(`http://localhost/api/task_delegation/delegations?taskIds=${taskId},${taskId},${otherId}`))).status).toBe(200)
...
it('rejects malformed IDs before loading data', async () => {
  await expect(GET(new Request('http://localhost/api/task_delegation/delegations?taskIds=bad'))).rejects.toThrow()
...
it('revalidates guard modifications and runs callbacks after a committed command', async () => {
  ...
  expect(mockExecute).toHaveBeenCalledWith('task_delegation.task.delegate', { input: { taskId, agentUserId: otherId }, ctx: mockContext.commandContext })
...
it('un-delegates only the task from the guarded route', async () => {
  const response = await DELETE(new Request(`http://localhost/api/task_delegation/delegations/${taskId}`, { method: 'DELETE' }), { params: Promise.resolve({ taskId }) })
  expect(response.status).toBe(200)
  expect(mockExecute).toHaveBeenCalledWith('task_delegation.task.undelegate', { input: { taskId }, ctx: mockContext.commandContext })
})

it('uses the scoped delegate service for agent options', async () => {
  expect((await agents(new Request('http://localhost/api/task_delegation/agents'))).status).toBe(200)
