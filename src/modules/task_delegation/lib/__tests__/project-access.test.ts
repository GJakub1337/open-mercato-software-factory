...
it('denies expired Staff membership consistently in AI tools, delegation reads and writes', async () => {
  ...
  await expect(aiTools.find((tool) => tool.name === 'task_delegation.get_delegation')!.handler({ taskId }, toolContext)).resolves.toEqual({ found: false })
  await expect(createTaskDelegationService({ em }).getDelegations(ctx, [taskId])).resolves.toEqual([])
