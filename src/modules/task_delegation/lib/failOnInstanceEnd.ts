...
export async function failDelegatedTask(payload: WorkflowEndPayload, context: SubscriberContext): Promise<void> {
  ...
  await context.resolve<CommandBus>('commandBus').execute('task_delegation.task.set_status', {
    input: {
      ...
      status: 'failed',
      reason: workflowInstance.errorMessage ?? payload.errorMessage ?? translate('task_delegation.errors.workflowEnded', 'Workflow ended before task completion.'),
    },
    ctx: commandContext,
  })
}
