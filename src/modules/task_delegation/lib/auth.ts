...
export async function requireTaskScope(ctx: CommandRuntimeContext): Promise<TaskScope> {
  ...
  if (!tenantId || !organizationId || !userId) {
    throw new CrudHttpError(403, { code: 'scope_required', error: translate('task_delegation.errors.scopeRequired', 'Tenant and organization scope are required.') })
  }
  if (ctx.organizationIds && !ctx.organizationIds.includes(organizationId)) {
    throw new CrudHttpError(403, { code: 'scope_required', error: translate('task_delegation.errors.scopeUnavailable', 'Organization scope is not available.') })
  }
...
export async function requireFeature(ctx: CommandRuntimeContext, feature: string): Promise<TaskScope> {
  ...
  try {
    ...
    const { translate } = await resolveTranslations()
    throw new CrudHttpError(403, { code: 'forbidden', error: translate('task_delegation.errors.permissionUnavailable', 'Permission could not be verified.') })
  }
  if (!await rbac.userHasAllFeatures(scope.userId, [feature], scope)) {
    const { translate } = await resolveTranslations()
    throw new CrudHttpError(403, { code: 'forbidden', error: translate('task_delegation.errors.forbidden', 'Permission denied.') })
  }
  return scope
}
