...
export const aiTools: AiToolDefinition[] = [
  defineAiTool<unknown, unknown>({
    name: 'task_delegation.get_delegation',
    ...
    inputSchema: delegationInput, requiredFeatures: ['task_delegation.view'], isMutation: false,
    async handler(raw, context) {
      const { taskId } = delegationInput.parse(raw)
      const [item] = await context.container.resolve<TaskDelegationService>('taskDelegationService')
