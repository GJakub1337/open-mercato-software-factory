...
function fixture(visible = true) {
  ...
  const context = {
    tenantId: 'tenant', organizationId: 'org', userId: 'user', userFeatures: ['task_delegation.view'], isSuperAdmin: false,
    container: { resolve: (name: string) => {
      if (name === 'taskDelegationService') return { getDelegations }
      throw new Error(name)
    } },
  } as unknown as McpToolContext
  return { context, getDelegations }
}
const tool = () => aiTools.find((item) => item.name === 'task_delegation.get_delegation')!

describe('tasks AI tools', () => {
  it('registers only the delegation read; task reads live in task_tools', () => {
    expect(aiTools.map((item) => item.name)).toEqual(['task_delegation.get_delegation'])
    expect(tool()).toMatchObject({ isMutation: false, requiredFeatures: ['task_delegation.view'] })
  })
