...
type Listener = { resolve: (item: TaskDelegationReadItem | null) => void; reject: (error: unknown) => void }
...
async function flush(): Promise<void> {
  ...
  await Promise.all(Array.from({ length: Math.ceil(ids.length / 100) }, async (_, page) => {
    ...
    try {
      const response = await readApiResultOrThrow<{ items: TaskDelegationReadItem[] }>(`/api/task_delegation/delegations?taskIds=${encodeURIComponent(taskIds.join(','))}`)
...
export function loadTaskDelegation(taskId: string): Promise<TaskDelegationReadItem | null> {
