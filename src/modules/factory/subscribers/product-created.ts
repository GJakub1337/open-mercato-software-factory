 ...
 * to Factory; the delegation starts `factory.deliver`. Scope comes only from the event-bus
...
export default async function handle(payload: ProductCreatedPayload, ctx: SubscriberContext): Promise<void> {
  ...
  if (result.status === 'skipped') logger.warn('factory intake skipped', { productId, ...result })
  else logger.info('factory intake on the board', { productId, sku: product.sku, ...result })
}
