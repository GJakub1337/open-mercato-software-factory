...
export async function approveProductTask(ctx: CommandRuntimeContext, taskId: string, github: GitHubClient): Promise<ApproveResult> {
  ...
  if (!task) throw await refuse(404, 'task_not_found', 'factory.approve.errors.notFound', 'Task not found.')
  ...
  if (current !== 'in-review' || !done) throw await refuse(409, 'not_in_review', 'factory.approve.errors.notInReview', 'Only a task in review can be approved.')
  ...
  if (members.items[0]?.user_id !== scope.userId) {
    throw await refuse(403, 'assignee_required', 'factory.approve.errors.assigneeOnly', 'Only the task assignee can approve the pull request.')
  }

  const pr = await github.getPullRequest(prNumber)
  if (!pr) throw await refuse(409, 'no_pull_request', 'factory.approve.errors.noPullRequest', 'The task has no website pull request yet.')
  const alreadyMerged = pr.merged
  if (!pr.merged) {
    if (pr.state === 'closed') throw await refuse(409, 'pr_closed', 'factory.approve.errors.prClosed', 'The pull request was closed without merging.')
    try {
      await github.mergePullRequest(pr.number, pr.headSha)
    } catch (error) {
      if (error instanceof GitHubApiError && [405, 409, 422].includes(error.status)) {
        throw await refuse(409, 'merge_blocked', 'factory.approve.errors.mergeBlocked', 'GitHub refused the merge: checks are not green or the pull request changed. Review it again.')
      }
      throw error
    }
  }
